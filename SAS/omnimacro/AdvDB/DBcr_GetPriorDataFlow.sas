%macro DBcr_GetPriorDataFlow(
	FileList
	,inETLFlow	=	WORK.__ETLcfg_Dats__
	,outETLFlow	=	WORK.__ETL_PriorDataFlow__
	,inValType	=	V
...
|100.	Introduction.																													|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|This macro is intended to retrieve a subset of the provided Data Flow, which can be executed independently from the beginning of	|
|	| source data collection to where the listed files/datasets are generated, with the call of the least necessary programs.			|
|---------------------------------------------------------------------------------------------------------------------------------------|
|200.	Glossary.																														|
|---------------------------------------------------------------------------------------------------------------------------------------|
...
|	|				Default : [WORK.__ETL_PriorDataFlow__]																				|
...
|300.	Update log.																														|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	| Date |	20181106		| Version |	1.00		| Updater/Creator |	Lu Robin Bin												|
...
%if	%length(%qsysfunc(compress(&outETLFlow.,%str( ))))	=	0	%then	%let	outETLFlow	=	WORK.__ETL_PriorDataFlow__;
...
%local
	...
	NextL		GnLST		Fi			tmpPgmLast	f_noNext
;
...
data &procLIB..__GPDF_inlist&mNest.;
...
proc sort
	...
	out=&procLIB..__GPDF_inETLflow&mNest.
	nodupkey
;
	by	&KeyOfFlow.;
run;
data &procLIB..__GPDF_inETLflow&mNest.;
	set &procLIB..__GPDF_inETLflow&mNest.;
	&C_FILENAME.	=	lowcase(&C_FILENAME.);
...
%*200.	Search for the last process steps in the ETL meta table which generate current files.;
%*210.	Find all processes that generate current files.;
proc sql;
	create table &procLIB..__GPDF_FileProc&mNest. as (
		select	a.*
		from &procLIB..__GPDF_inETLflow&mNest.( where=( upcase(&C_FlowType.) = "OUTPUT" ) ) as a
		inner join &procLIB..__GPDF_inlist&mNest. as b
			on	strip(a.&C_FILENAME.)	=	strip(b.&C_FILENAME.)
	);
quit;

%*219.	Quit the search if none of the files is generated in current ETL process.;
%if	%getOBS4DATA( inDAT = &procLIB..__GPDF_FileProc&mNest. , gMode = F )	=	0	%then %do;
	%*199.	Debugger.;
	%if	&fDebug.	=	1	%then %do;
		%put	%str(I)NFO: [&L_mcrLABEL.][Nest Layer:&mNest.]Quit the search as none of the files is generated in current ETL process;
	%end;
...
proc sort
	data=&procLIB..__GPDF_FileProc&mNest.
;
	by
		&C_FILENAME.
		&FlowByVar.
...
run;
data &procLIB..__GPDF_FileProc&mNest.;
	set &procLIB..__GPDF_FileProc&mNest.;
	by
		&C_FILENAME.
		&FlowByVar.
	;
...
%*290.	Issue message when some of the provided files are NOT generated in current ETL process.;
proc sql;
	create table &procLIB..__GPDF_FileMiss&mNest. as (
		select	a.*
		from &procLIB..__GPDF_inlist&mNest. as a
		left join &procLIB..__GPDF_FileProc&mNest. as b
			on	strip(a.&C_FILENAME.)	=	strip(b.&C_FILENAME.)
		where missing(b.&C_FILENAME.)	=	1
	);
quit;
%if	%getOBS4DATA( inDAT = &procLIB..__GPDF_FileMiss&mNest. , gMode = F )	=	0	%then %do;
	%put	%str(N)OTE: [&L_mcrLABEL.][Nest Layer:&mNest.]Some of the provided files are NOT generated in current ETL process, see [&procLIB..__GPDF_FileMiss&mNest.];
%end;

%*300.	Identify all files in the same process steps.;
data &procLIB..__GPDF_ProcIn&mNest.;
	%*100.	Set the meta table.;
	set &procLIB..__GPDF_inETLflow&mNest. end=EOF;
	by	&FlowByVar.;

	%*200.	Prepare to load the program names.;
	if	_N_	=	1	then do;
		dcl	hash	hPGM(dataset:"&procLIB..__GPDF_FileProc&mNest.",multidata:"N");
		hPGM.DefineKey("&C_PGMNAME.");
		hPGM.DefineData("&C_PGMNAME.");
		hPGM.DefineDone();
	end;

	%*300.	Prepare to identify the very last program to cut the ETL meta table for the next round of recursion.;
	array	tmpPGM{1}	$32767	_temporary_;
	...
	%*900.	Retrieve the very last program for current round of search.;
	if	EOF	then do;
		call symputx( "tmpPgmLast" , tmpPGM{1} , "L" );
	end;
run;

%*309.	Debugger.;
%if	&fDebug.	=	1	%then %do;
	%put	%str(I)NFO: [&L_mcrLABEL.][Nest Layer:&mNest.]All steps BEFORE this program in the ETL process are used as the base for the next round of recursive search if any [tmpPgmLast=%superq(tmpPgmLast)];
%end;

%*390.	Skip recursive search if none of current programs has input file.;
%if	%getOBS4DATA( inDAT = %nrbquote( &procLIB..__GPDF_ProcIn&mNest.( where=( upcase(&C_FlowType.) = "INPUT" ) ) ) , gMode = F )	=	0	%then %do;
	%*100.	Mark that there should be no further recursive search.;
	%let	f_noNext		=	1;

	%*199.	Debugger.;
	%if	&fDebug.	=	1	%then %do;
		%put	%str(I)NFO: [&L_mcrLABEL.][Nest Layer:&mNest.]None of current programs has input file, see [&procLIB..__GPDF_ProcIn&mNest.];
	%end;
...
%*400.	Prepare a subset of the ETL meta table with all processes BEFORE the very last program identified at above steps.;
data &procLIB..__GPDF_ETLsub&mNest.;
	%*100.	Set the meta table.;
	set &procLIB..__GPDF_inETLflow&mNest.;
	by	&FlowByVar.;
	retain	f_keep	1;
	...
	if	&C_PGMNAME.	=	%sysfunc(quote( %superq(tmpPgmLast) , %str(%') ))	then do;
		f_keep	=	0;
	end;

	%*300.	Remove all later steps.;
	if	f_keep	=	0	then	delete;

	%*900.	Purge.;
	drop	f_keep;
run;

%*490.	Skip recursive search if none of current programs has prior process.;
%if	%getOBS4DATA( inDAT = &procLIB..__GPDF_ETLsub&mNest. , gMode = F )	=	0	%then %do;
	%*100.	Mark that there should be no further recursive search.;
	%let	f_noNext		=	1;

	%*199.	Debugger.;
	%if	&fDebug.	=	1	%then %do;
		%put	%str(I)NFO: [&L_mcrLABEL.][Nest Layer:&mNest.]None of current programs has prior process, see [&procLIB..__GPDF_ProcIn&mNest.];
	%end;
...
%*500.	Verify whether the [INPUT] files at current steps are also [OUTPUT] files at the prior steps.;
%*510.	Search for the files.;
proc sql;
	create table &procLIB..__GPDF_hasRecur&mNest. as (
		select	a.*
		from &procLIB..__GPDF_ETLsub&mNest.( where=( upcase(&C_FlowType.) = "OUTPUT" ) ) as a
		inner join &procLIB..__GPDF_ProcIn&mNest.( where=( upcase(&C_FlowType.) = "INPUT" ) ) as b
			on	strip(a.&C_FILENAME.)	=	strip(b.&C_FILENAME.)
	);
quit;

%*590.	Skip recursive search if all the input files of current programs directly come from outside the ETL process.;
%if	%getOBS4DATA( inDAT = &procLIB..__GPDF_hasRecur&mNest. , gMode = F )	=	0	%then %do;
	%*100.	Mark that there should be no further recursive search.;
	%let	f_noNext		=	1;

	%*199.	Debugger.;
	%if	&fDebug.	=	1	%then %do;
		%put	%str(I)NFO: [&L_mcrLABEL.][Nest Layer:&mNest.]All input files of current programs directly come from outside the ETL process, see [&procLIB..__GPDF_ProcIn&mNest.];
	%end;
...
%*800.	Search for the parent processes by calling the same function in recursion.;
%DBcr_GetPriorDataFlow(
	%nrbquote( &procLIB..__GPDF_ProcIn&mNest.( where=( upcase(&C_FlowType.) = "INPUT" ) ) )
	,inETLFlow	=	&procLIB..__GPDF_ETLsub&mNest.
	,outETLFlow	=	&procLIB..__GPDF_ProcPrior&mNest.
	,inValType	=	D
	,C_FILENAME	=	&C_FILENAME.
	,C_PGMNAME	=	&C_PGMNAME.
	,C_FlowType	=	&C_FlowType.
...
%*850.	We have to combine the current program that generates the dedicated files with all its prior steps as the final chain.;
%SkipSearch:
data &procLIB..__GPDF_ProcFnl&mNest.;
	set
		&procLIB..__GPDF_ProcIn&mNest.
	%if	&f_noNext.	=	0	%then %do;
		&procLIB..__GPDF_ProcPrior&mNest.
	%end;
	;
run;
...
%if	&fDebug.	=	1	%then %do;
	%if	&f_noNext.	=	0	%then %do;
		%put	%str(I)NFO: [&L_mcrLABEL.][Nest Layer:&mNest.]The result comes from recursive search, combined by [&procLIB..__GPDF_ProcIn&mNest.] and [&procLIB..__GPDF_ProcPrior&mNest.];
	%end;
	%else %do;
		%put	%str(I)NFO: [&L_mcrLABEL.][Nest Layer:&mNest.]The result comes directly from [&procLIB..__GPDF_ProcIn&mNest.];
	%end;
%end;

%*890.	Dedup as the provided files may share the same prior processes.;
proc sort
	data=&procLIB..__GPDF_ProcFnl&mNest.
...
;
	delete
		__GPDF_inlist&mNest.
		__GPDF_inETLflow&mNest.
		__GPDF_FileProc&mNest.
		__GPDF_ETLsub&mNest.
		__GPDF_hasRecur&mNest.
		__GPDF_ProcPrior&mNest.
		__GPDF_ProcFnl&mNest.
	;
run;
quit;
...
%DBcr_GetPriorDataFlow(
	%nrstr( db.fnl_dates | db.ChangeJobs )
	,inETLFlow	=	test
	,outETLFlow	=	WORK.__ETL_PriorDataFlow__
	,inValType	=	V
