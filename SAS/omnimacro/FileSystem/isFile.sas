...
|100.	Introduction.																													|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|This macro is intended to identify whether the input OS member is a File instead of a Directory.									|
|---------------------------------------------------------------------------------------------------------------------------------------|
|200.	Glossary.																														|
|---------------------------------------------------------------------------------------------------------------------------------------|
...
%*200.	Try to open the member as file.;
%let	filID	=	%sysfunc(fopen(&filref.));

%*300.	Assign the output result if it is successfully opened as a file.;
%if	%eval( &filID. > 0 )	%then %do;
	%*100.	Close the member as purge.;
	%let	filrc	=	%sysfunc(fclose(&filID.));
...
%*100.	Testing.;
%put	%isFile(C:\Program Files);
%put	%isFile(C:\Program Files2);
%put	%isFile(C:\Program Files\Common Files\System\DirectDB.dll);
