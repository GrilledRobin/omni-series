%macro usFUN_LTrimByChar;
%*000.	Info.;
/*-------------------------------------------------------------------------------------------------------------------------------------*\
|100.	Introduction.																													|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|This macro is intended to left-trim the character string by the specified character.												|
|	|The most common usage is to remove the leading 0 in a serial number as [000000681581] and return [681581] as the result.			|
|	|-----------------------------------------------------------------------------------------------------------------------------------|
...
;
	...
	%*300.	Define the pattern to remove the leading characters as specified.;
	tmpPRX	=	prxparse(cats('s/^',ifc(index('.\*+?()[]{}|^$#',strip(inCHAR)),'\',''),strip(inCHAR),'*(.*)$/\1/ismx'));
...
%put	%sysfunc(LTrimByChar(0006805660000,0));
%put	%sysfunc(LTrimByChar($$$$$$$15114$,$));
