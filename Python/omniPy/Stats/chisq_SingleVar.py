...
def chisq_SingleVar( inDAT , dependent , response , event = 1 ) -> tuple:
    #000. Info.
    '''
#---------------------------------------------------------------------------------------------------------------------------------------#
#100.   Introduction.                                                                                                                   #
...
#   |               [outChiSq] :   <np.float64> The Chi-Square value upon <response> for <dependent>                                    #
#   |               [outDoF  ] :   <int       > The degree of Freedom in terms of unique values in <dependent>                          #
#   |               [ourPVal ] :   <np.float64> The P-Value at such Chi-Square distribution with such Degree of Freedom                 #
#---------------------------------------------------------------------------------------------------------------------------------------#
#300.   Update log.                                                                                                                     #
#---------------------------------------------------------------------------------------------------------------------------------------#
...
#---------------------------------------------------------------------------------------------------------------------------------------#
    ...
    if k_Event == 0:
        raise ValueError(
            ...
            + f' Chi-Square test fails for the response:[{response}] in terms of the dependent:[{dependent}]!'
        )
    if k_NonEvent == 0:
        raise ValueError(
...
if __name__=='__main__':
    ...
    l_resi : list = [
        [u'上海',0.427] , [u'北京',0.332] , [u'杭州',0.058] , [u'天津',0.125] , [u'重庆',0.014] , [u'成都',0.007]
        , [u'广州',0.009] , [u'深圳',0.001] , [u'厦门',0.027]
    ]
