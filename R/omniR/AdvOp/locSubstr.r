...
#100.   Introduction.                                                                                                                   #
#---------------------------------------------------------------------------------------------------------------------------------------#
#   |This function is intended to get the start and end of substrings matching <ptn> in the provided <txt>, with or without overlapping #
#   |-----------------------------------------------------------------------------------------------------------------------------------#
#   |[QUOTE]                                                                                                                            #
#   |-----------------------------------------------------------------------------------------------------------------------------------#
...
#   |100.   Parameters.                                                                                                                 #
#   |-----------------------------------------------------------------------------------------------------------------------------------#
#   |ptn        :   Regular expression used to search for substrings                                                                    #
...
#   |...        :   Other arguments for <stringi::stri_locate_all_regex>, please check its official document                            #
#   |-----------------------------------------------------------------------------------------------------------------------------------#
#   |900.   Return Values by position.                                                                                                  #
#   |-----------------------------------------------------------------------------------------------------------------------------------#
#   |<list>     :   List of matrices, indicating <[start, end]> of each match of <ptn>, both inclusive                                  #
#---------------------------------------------------------------------------------------------------------------------------------------#
#300.   Update log.                                                                                                                     #
#---------------------------------------------------------------------------------------------------------------------------------------#
...
#   | Date |    20260110        | Version | 1.10        | Updater/Creator | Lu Robin Bin                                                #
#   |______|____________________|_________|_____________|_________________|_____________________________________________________________#
#   | Log  |[1] Changed the first argument to <ptn> to avoid naming conflict                                                            #
#   |      |[2] Introduced dots <...> to take over the rest of arguments from <stringi::stri_locate_all_regex>, for more compatibility  #
#   |______|____________________________________________________________________________________________________________________________#
#---------------------------------------------------------------------------------------------------------------------------------------#
#400.   User Manual.                                                                                                                    #
#---------------------------------------------------------------------------------------------------------------------------------------#
...
locSubstr <- function(
	...
	if (overlap) {
		...
		#[ASSUMPTION]
		#[1] We have to avoid non-capturing groups here `(?:...)`, otherwise the result is unexpected
		rx <- paste0('(?=(', ptn, '))')

		#300. Correct dots to avoid duplicated parameters
		dots <- rlang::list2(...)
		if (length(names(dots)) > 0){
			dots <- dots[!(names(dots) %in% c('capture_groups'))]
		}

		#500. Locate the matches by extracting the [groups] as well
		str_loc <- do.call(stringi::stri_locate_all_regex, c(txt, rx, capture_groups = T, dots))
		...
		return(stringi::stri_locate_all_regex(txt, ptn, ...))
	}
}
