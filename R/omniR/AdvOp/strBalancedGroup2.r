...
#   |CAVEAT                                                                                                                             #
#   |-----------------------------------------------------------------------------------------------------------------------------------#
#   |[1] Despite of its high efficiency, this function can only handle cases with below features at the same time:                      #
#   |    [1] Single pair of enclosers to track                                                                                          #
#   |    [2] All enclosers are in pairs, with no isoloated <opener> or <closer>                                                         #
#   |    [3] No need of META information, e.g. the spans, positions, etc.                                                               #
#   |[2] It cannot recognize the positions where there are isolated enclosers                                                           #
#---------------------------------------------------------------------------------------------------------------------------------------#
#200.   Glossary.                                                                                                                       #
#---------------------------------------------------------------------------------------------------------------------------------------#
#   |100.   Parameters.                                                                                                                 #
...
#   |include           :   <logical > Whether to include the enclosers in the output structure.                                         #
...
#   |               [IMPORTANT] If the bounds do not exist in pairs, <NA> is returned                                                   #
#---------------------------------------------------------------------------------------------------------------------------------------#
#300.   Update log.                                                                                                                     #
#---------------------------------------------------------------------------------------------------------------------------------------#
...
strBalancedGroup2 <- function(
	...
	lBound <- names(enclosers)
	rBound <- unname(enclosers)
	...
	getstr <- function(i){
		...
		#300. Add 1 on group ID if we encounter the <opener>, or subtract by 1 if we encounter the <closer>
...
if (FALSE){
	...
	if (TRUE){
		...
		chkrst <- strBalancedGroup2(chkstr, enclosers = list('<div.*?>' = '</div>'), rx = T)
		...
		chkrst2 <- strBalancedGroup2(chkstr, enclosers = list('<div.*?>' = '</div>'), rx = T, include = F)
		...
		nestedFormatter(strBalancedGroup2(''))
		# list(
		#   chr. (NA)
		# )

		nestedFormatter(strBalancedGroup2('a'))
		# list(
		#   chr. (NA)
		# )

		nestedFormatter(strBalancedGroup2('(a b)'))
		# list(
		#   chr. ("(a b)")
		# )
		...
		nestedFormatter(strBalancedGroup2('(a ((b) c (d))) e (f (g))'))
		# list(
		#   chr. (
		#     "(a ((b) c (d)))"
		#     ,"(f (g))"
		...
		nestedFormatter(strBalancedGroup2('(a ((b) c (d))) e (f (g))', include = F))
		# list(
		#   chr. (
		#     "a ((b) c (d))"
		#     ,"f (g)"
		...
		t_overall <- 0.0
		t1 <- lubridate::now()
		bg_large <- strBalancedGroup2(str_large, enclosers = c('<div.*?>' = '</div>'), rx = T)
		t2 <- lubridate::now()
