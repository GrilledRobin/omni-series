...
|100.	Introduction.																													|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|This macro is intended to return the type of the specified variable in the given dataset.											|
|---------------------------------------------------------------------------------------------------------------------------------------|
|200.	Glossary.																														|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|inDAT		:	Input dataset.																										|
|	|inFLD		:	The variable or field name to be verified.																			|
|	|outVAR		:	Macro variable name that holds the Type of the specified variable or field.											|
...
%if	&DSID.	=	0	%then %do;
	...
	%goto	MEXIT;
%end;
%let	&outVAR.	=	%sysfunc(vartype(&DSID.,%sysfunc(varnum(&DSID.,&inFLD.))));
