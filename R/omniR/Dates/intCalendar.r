...
#   |100.   Parameters.                                                                                                                 #
#   |-----------------------------------------------------------------------------------------------------------------------------------#
#   |interval    :   Either character string or a valid <list> holding items generated by function <Dates$getDateIntervals>             #
#   |                 value, while the functions raises error if it is NOT provided.                                                    #
#   |                 [<chr>       ]           See definition of <Dates$getDateIntervals> for accepted values                           #
...
#   |                 [W           ]           Working Days                                                                             #
#   |                 [T           ]           Trading Days                                                                             #
#   |col_rowidx  :   Numeric column name representing the record index within the whole output calendar                                 #
#   |                 [.ical_row.  ] <Default> The default column name                                                                  #
#   |                 [<chr>       ]           Any valid character string as column name                                                #
#   |col_period  :   Numeric column name representing the <period> in the output calendar                                               #
#   |                 [.ical_prd.  ] <Default> The default column name                                                                  #
#   |                 [<chr>       ]           Any valid character string as column name                                                #
#   |col_prdidx  :   Numeric column name representing the record index within each <period> in the output calendar                      #
#   |                 [.ical_rprd. ] <Default> The default column name                                                                  #
#   |                 [<chr>       ]           Any valid character string as column name                                                #
#   |col_weekday :   Logical column name representing the record flag of <weekday> in the output calendar                               #
...
intCalendar <- function(
	...
	h_dateidx <- function(df, firstrec) {
		...
		if ((dict_attr[['itype']] %in% c('d', 'dt')) & (daytype %in% c('W', 'T'))) {
			rst %<>% dplyr::group_by_at(dplyr::all_of(col_period)) %>%
	...
	if (dict_attr[['span']] == 1) {
		...
		if (dict_attr[['name']] %in% c('weekday', 'dtweekday')) {
			...
			outRst[[col_weekday]] <- outRst %>%
				...
				dplyr::pull(dplyr::all_of(col_out))
