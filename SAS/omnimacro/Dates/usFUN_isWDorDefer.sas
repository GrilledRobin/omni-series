%macro usFUN_isWDorDefer;
%*000.	Info.;
/*-------------------------------------------------------------------------------------------------------------------------------------*\
|100.	Introduction.																													|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|This function is intended to return the provided date value if it is Workday, or return its next Workday if it is not,				|
...
|	|-----------------------------------------------------------------------------------------------------------------------------------|
|	|[(Number)]	:	The date value that denotes [inDATE] (if it is Workday) or its Next Workday.										|
...
;
	...
	if	isWorkDay(inCalendar,ClnDateVar,ClnFWDVar,inDATE)	then do;
		...
		return(nextWorkday(inCalendar,ClnDateVar,ClnFWDVar,inDATE));
	end;
...
data bb;
	...
	D_WD	=	isWDorDefer("clndr.calendar2015","D_DATE","F_WORKDAY",D_NTB);
run;
