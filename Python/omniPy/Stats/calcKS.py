...
def calcKS( inDAT , dependent , response , event = 1 ) -> tuple:
    #000. Info.
    """
#---------------------------------------------------------------------------------------------------------------------------------------#
#100.   Introduction.                                                                                                                   #
...
#---------------------------------------------------------------------------------------------------------------------------------------#
    ...
    r_pos : np.float64 = k_Event / ( k_Event + k_NonEvent )
    ...
    if k_Event == 0:
        raise ValueError(
            ...
            + f'KS means nothing to the response:[{response}] in terms of the dependent:[{dependent}]!'
        )
    if k_NonEvent == 0:
        raise ValueError(
    ...
    #320. Calculate the cummulative percentage for [Event], [NonEvent] and [Total] respectively.
    #[ASSUMPTION]
    #[1] Stats: 分别计算TP，FP与当前[预测为正例]的累计百分比
    cumdens = (
        ( tabfreq.cumsum() / tabfreq.sum() )
        .rename( columns = { 'NonEvent':'cp_NonEvent' , 'Event':'cp_Event' , 'Total':'cp_Total' } )
    )
    ...
    #350. Crate a smooth value for the cummulative density.
    #[ASSUMPTION]
    #[1] Stats: 对累计百分比做连续两组的平滑处理)(Exp: This is implemented in some algorithms.)
    ...
    outKS : np.float64 = cumdens[ cumdens['TPR_FPR_Gap'] == cumdens['TPR_FPR_Gap'].max() ]['TPR_FPR_Gap'][0]

    #600. Calculate Accuracy Ratio.
    #[ASSUMPTION]
    #[1] This is implemented in some algorithms.
    #[2] 用当前decile中平滑处理过的TPR与同样处理过的[预测正例在全数据中的比重]做差值，再用[当前decile的样本在全数据中的比重]进行加权
    ...
    ourAR : np.float64 = abs( sum( cumdens['AR'] ) / ( ( 1 - r_pos )/2 ) )
...
#[10] 同样以前10%的数值作为阈值，即将前10%的实例都划归为正类；则正确预测到的正例数占预测正例总数的比例：PV = [Event] / ( [Event] + [NonEvent] )
#      (若以前20%的数值作为阈值，则上述计算需用[累计数])
