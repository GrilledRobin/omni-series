...
|100.	Introduction.																													|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|This macro is intended to return the length of the specified variable in the given dataset.										|
|---------------------------------------------------------------------------------------------------------------------------------------|
|200.	Glossary.																														|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|inDAT		:	Input dataset.																										|
|	|inFLD		:	The variable or field name to be verified.																			|
|	|outVAR		:	Macro variable name that holds the Length of the specified variable or field.										|
...
%if	&DSID.	=	0	%then %do;
	...
	%goto	MEXIT;
%end;
%let	&outVAR.	=	%sysfunc(varlen(&DSID.,%sysfunc(varnum(&DSID.,&inFLD.))));
