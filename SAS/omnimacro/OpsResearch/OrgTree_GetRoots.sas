...
|100.	Introduction.																													|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|This macro is intended to retrieve all roots in an Organizational Tree, which means there are no superiors linked to them.			|
|---------------------------------------------------------------------------------------------------------------------------------------|
|200.	Glossary.																														|
|---------------------------------------------------------------------------------------------------------------------------------------|
...
|	|outVAR		:	The output variable that denotes to the roots in the tree.															|
...
%if	%length(%qsysfunc(compress(&inDAT.,%str( ))))	=	0	%then %do;
	...
	%ErrMcr
%end;
%if	%length(%qsysfunc(compress(&outVAR.,%str( ))))	=	0	%then	%let	outVAR		=	TreeRoot;
...
%*200.	Retrieve the unique [VarUpper] in the input dataset.;
proc freq
	data=%unquote(&inDAT.)
	noprint
;
...
data %unquote(&outDAT.);
	...
	%*100.	Set the unique [VarUpper].;
	set	&procLIB.._ot_gr;
	&outVAR.	=	&VarUpper.;

	%*200.	Prepare the HASH object to identify whether current [VarUpper] acts as [VarLower] of other branches.;
	if	_N_	=	1	then do;
		dcl	hash	hDirSup(dataset:"&inDAT.",hashexp:16);
		hDirSup.DefineKey("&VarLower.");
		hDirSup.DefineData("&VarLower.");
		hDirSup.DefineDone();
	end;
	call missing(&VarLower.);

	%*200.	Only output if current [VarUpper] does not act as [VarLower] of any other branches.;
	if	hDirSup.check(key:&outVAR.)	^=	0	then do;
		output;
	end;
