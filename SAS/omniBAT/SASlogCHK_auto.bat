...
if exist "%CURR_PATH%\%LOG_NAME%" (
	...
	@sas "%errCHKSAScfg%" %INITCFG% -nologo -noautoexec -icon
)
...
if exist "!CURR_PATH!\errCHK!CDW_TODAY!.log" (
	for /f %%i in ('dir /A-D /B "!CURR_PATH!\errCHK!CDW_TODAY!.log"') do (
		...
		if "!LOGSIZE!"=="0" (
			...
			::QUOTE: https://www.tutorialspoint.com/batch_script/batch_script_return_code.htm
			::We do not use [/B] as we need to quite the caller [CMD.EXE] in line
			@exit -2
		)
	)
)
