...
|	|-----------------------------------------------------------------------------------------------------------------------------------|
|	|[1]	:	The member is a Directory that physically EXIST on the harddrive.														|
|	|[0]	:	The member is NOT a Directory, but physically EXIST on the harddrive.													|
...
%*		put	PutMsg;
	...
	%*300.	Try to open the member as directory.;
	filID	=	dopen(filref);

	%*400.	Return TRUE if it is successfully opened as a directory.;
	if	filID	>	0	then do;
...
%put	%sysfunc(isDir(C:\Program Files));
%put	%sysfunc(isDir(C:\Program Files2));
%put	%sysfunc(isDir(C:\Program Files\Common Files\System\DirectDB.dll));
