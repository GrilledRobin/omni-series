%macro usFUN_StripByChar;
%*000.	Info.;
/*-------------------------------------------------------------------------------------------------------------------------------------*\
|100.	Introduction.																													|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|This macro is intended to remove the leading and trailing specified character from the given character string.						|
|	|The most common usage is to remove the leading and trailing [_] in a string as [___fdai3__] and return [fdai3] as the result.		|
|	|-----------------------------------------------------------------------------------------------------------------------------------|
...
|	|inCHAR	:	The character to be removed from both sides of the input character string.												|
|---------------------------------------------------------------------------------------------------------------------------------------|
|300.	Update log.																														|
|---------------------------------------------------------------------------------------------------------------------------------------|
...
;
	...
	tmpPRX	=	prxparse(cats('s/^',ifc(index('.\*+?()[]{}|^$#',strip(inCHAR)),'\',''),strip(inCHAR),'*(.*?)',ifc(index('.\*+?()[]{}|^$#',strip(inCHAR)),'\',''),strip(inCHAR),'*\s*$/\1/ismx'));
...
%put	%sysfunc(StripByChar(0006805660000,0));
%put	%sysfunc(StripByChar($$$$$$$15114$,$));
