...
|100.	Introduction.																													|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|This macro is intended to retrieve the character attributes for given dataset.														|
|---------------------------------------------------------------------------------------------------------------------------------------|
|200.	Glossary.																														|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|inDAT	:	Input dataset.																											|
|	|inATTR	:	Character attribute or alias, should be the same as what the function ATTRC() requires.									|
...
|300.	Update log.																														|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	| Date |	20130715		| Version |	1.00		| Updater/Creator |	Lu Robin Bin												|
...
%if	%length(%qsysfunc(compress(&inATTR.,%str( ))))	=	0	%then %do;
	%put	%str(W)ARNING: [&L_mcrLABEL.]No Attirbute is provided for extraction!;
	%goto	MEXIT;
%end;
...
%if	&DSID.	=	0	%then %do;
	...
	%goto	MEXIT;
%end;
%let	&outVAR.	=	%sysfunc(attrc(&DSID.,&inATTR.));
