...
|100.	Introduction.																													|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|This macro is intended to update the Category Count dataset by the provided Category.												|
|	|There are 2 ways to update the Category Count dataset:																				|
|	|[1] Provide the values as character strings.																						|
|	|[2] Provide the dataset that holds the Categories, as well as their repective Frequency Counts.									|
|---------------------------------------------------------------------------------------------------------------------------------------|
|200.	Glossary.																														|
|---------------------------------------------------------------------------------------------------------------------------------------|
...
|300.	Update log.																														|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	| Date |	20170806		| Version |	1.00		| Updater/Creator |	Lu Robin Bin												|
...
|	|All the Categories will be translated into lower case.																				|
|	|Special characters %nrstr(&) and %nrstr(%%) cannot be set as the values in [inCategory], for the compilation						|
...
%if	%length(%qsysfunc(compress(&outDat.,%str( ))))		=	0	%then	%let	outDat		=	WORK.CC;
...
%*090.	Check the types of the Category variables in the output dataset.;
...
%*310.	Split the data.;
data
	&procLIB..__mdl_incc_upd
	&procLIB..__mdl_incc_ins
;
	%*100.	Set the data.;
	set	&procLIB..__mdl_incc_mns;
	...
	if	_N_	=	1	then do;
		...
		hBase.DefineKey("&outDat_Cat.");
		hBase.DefineData("&outDat_Freq.");
		hBase.DefineDone();
	end;

	%*500.	Split the dataset in terms of whether the Category is found in the base dataset.;
	if	hBase.check(key:&inCategory.)	^=	0	then do;
		output	&procLIB..__mdl_incc_ins;
	end;
	else do;
...
%end;
	...
	if	_N_	=	1	then do;
		if	0	then	set	&procLIB..__mdl_incc_upd;
		dcl	hash	hUpd(dataset:"&procLIB..__mdl_incc_upd",hashexp:16);
		hUpd.DefineKey("&inCategory.");
		hUpd.DefineData("&inFreq.");
		hUpd.DefineDone();
	end;
	if	ins	=	0	then do;
	...
	%*300.	Update the value if the same Category exists.;
	%*301.	Skip the Update process if there is no new value found.;
	if	hUpd.check(key:&outDat_Cat.)	^=	0	then do;
		goto	EndOfUpd;
	end;
	...
	_iorc_			=	hUpd.find(key:&outDat_Cat.);
...
data upd;
	length
		...
		Count		8
	;
	Category	=	"good";	Count	=	1;	output;
	Category	=	"gooD";	Count	=	1;	output;
	Category	=	"bad";	Count	=	1;	output;
run;
%Mdl_inCC(D,inDAT=upd)

%*200.	Update with special characters.;
%*There will be warnings reading that [D is not resolved].;
%Mdl_inCC(V,'b&d')
