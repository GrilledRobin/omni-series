%macro usFUN_nextWorkday;
%*000.	Info.;
/*-------------------------------------------------------------------------------------------------------------------------------------*\
|100.	Introduction.																													|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|This function is intended to return the next Workday of the provided date value,													|
...
|	|				The variable as can be denoted MUST be formatted as Date for the process by VVALUEX.								|
...
|	|-----------------------------------------------------------------------------------------------------------------------------------|
|	|[(Number)]	:	The date value that denotes the Next Workday of [inDATE].															|
...
;
	...
	do until (i = dim(_clnARR,1));
		%*100.	Skip to the next date if the current one is NOT later than the provided one.;
		if	_clnARR{i,1}	<=	inDATE	then do;
			goto	arrNextDT;
		end;

		%*200.	Skip to the next date if the current one is NOT Workday.;
		if	_clnARR{i,2}	=	0	then do;
			goto	arrNextDT;
		end;

		%*500.	Identify the closest Workday next to the provided one.;
		oDate	=	min(oDate,_clnARR{i,1});
...
%*400.	Call the function to retrieve the next Workdays for each observation in the provided data.;
data aa;
...
data bb;
	...
	D_WD	=	nextWorkday("clndr.calendar2015","D_DATE","F_WORKDAY",D_NTB);
run;

%*500.	Call the function to retrieve the next Workday for the provided date in Macro Facility.;
%put	%sysfunc(nextWorkday(clndr.calendar2015,D_DATE,F_WORKDAY,20414));
