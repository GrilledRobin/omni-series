...
|100.	Introduction.																													|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|This macro is intended to return the INFORMAT of the specified variable in the given dataset.										|
|---------------------------------------------------------------------------------------------------------------------------------------|
|200.	Glossary.																														|
|---------------------------------------------------------------------------------------------------------------------------------------|
|	|inDAT		:	Input dataset.																										|
|	|inFLD		:	The variable or field name to be verified.																			|
|	|outVAR		:	Macro variable name that holds the INFORMAT of the specified variable or field.										|
...
%if	&DSID.	=	0	%then %do;
	...
	%goto	MEXIT;
%end;
%let	&outVAR.	=	%sysfunc(varinfmt(&DSID.,%sysfunc(varnum(&DSID.,&inFLD.))));
