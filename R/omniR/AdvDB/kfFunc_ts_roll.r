...
myfunc <- deco$wrap(function(
	...
	if (!is.numeric(kDays)) {
		stop(glue::glue('[{LfuncName}]<kDays> must be a single positive integer!'))
	}
	if (length(kDays) != 1) {
		stop(glue::glue('[{LfuncName}]<kDays> must be a single positive integer!'))
	}
	kDays <- as.integer(kDays)
	if (kDays < 1) {
		stop(glue::glue('[{LfuncName}]<kDays> must be a single positive integer!'))
	}
	...
	if (genPHMul) {
		...
		chkEnd_d <- intnx('day', dateEnd_d, -1, daytype = indMod, kw_cal = kw_cal)
	} else {
	...
	if (fDebug){
		...
		message(glue::glue('[{LfuncName}][chkBgn]=[{chkBgn}]'))
	}
...
if (FALSE){
	...
	time_bgn <- Sys.time()
	rst <- do.call(kfFunc_ts_roll, args_ts_roll)
	time_end <- Sys.time()
	print(time_end - time_bgn)
	# Time difference of 5.533556 secs

	#400. Verify the result
	#410. Retrieve the newly created data
	file_kpi1 <- glue::glue('D:\\Temp\\r15_{G_d_rpt}.RData')
	rst_kpi1 <- std_read_R(file_kpi1, 'kpi1') %>%
		dplyr::filter(!!rlang::sym('C_KPI_ID') == '130102')
	rst_kpi2 <- std_read_R(file_kpi1, 'kpi2') %>%
		dplyr::filter(!!rlang::sym('C_KPI_ID') == '140112')

	#420. Prepare unanimous arguments
	cln <- UserCalendar$new( intnx('day', G_d_rpt, 1 - k_roll_days, 'b', daytype = 'c'), G_d_rpt )
	byvar_kpis <- args_ts_roll[['byVar']] %>% unlist() %>% unname() %>% c('C_KPI_ID')
	aggvar_kpis <- args_ts_roll[['aggrVar']]
	...
	time_bgn <- Sys.time()
	rst2 <- do.call(kfFunc_ts_roll, args_ts_roll2)
	time_end <- Sys.time()
	print(time_end - time_bgn)
	# Time difference of 5.467553 secs

	#700. Verify the result for the next workday
	#710. Retrieve the newly created data
	file_kpi1_2 <- glue::glue('D:\\Temp\\r15_{G_d_next}.RData')
	...
	cln2 <- UserCalendar$new( intnx('day', G_d_next, 1 - k_roll_days, 'b', daytype = 'c'), G_d_next )
