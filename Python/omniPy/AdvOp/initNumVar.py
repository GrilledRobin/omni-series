...
def initNumVar(
    ...
    ,inplace : bool = False
    ,UniVal : float = 0
) -> 'Initialize all numeric variables in a DataFrame with the value of zero when any of them is NULL':
    #000.   Info.
    """
#---------------------------------------------------------------------------------------------------------------------------------------#
#100.   Introduction.                                                                                                                   #
#---------------------------------------------------------------------------------------------------------------------------------------#
#   |This function is intended to replace the NULL values of all numeric variables in the given DataFrame with the provided one as      #
#   | unification.                                                                                                                      #
#---------------------------------------------------------------------------------------------------------------------------------------#
#200.   Glossary.                                                                                                                       #
#---------------------------------------------------------------------------------------------------------------------------------------#
...
#300.   Update log.                                                                                                                     #
#---------------------------------------------------------------------------------------------------------------------------------------#
#   | Date |    20190327        | Version | 1.00        | Updater/Creator | Lu Robin Bin                                                #
...
#---------------------------------------------------------------------------------------------------------------------------------------#
    ...
    import numpy as np
    import sys
    from omniPy.AdvOp import selNumVar
    ...
    cols : list = selNumVar( inDAT )

    #200.   Count the variables as found.
    ncol : int = len( cols )

    #200.   Create a list of values to be replaced during the initialization.
    reps : list = [ None , np.nan , np.inf , -np.inf ]
...
if __name__=="__main__":
    ...
    testNaN : pd.DataFrame = pd.DataFrame( { 'Num':[ np.nan , 1 ] , 'Char':[ 'aa' , np.nan ] } )

    #200.   Create a new dataset with the numeric values initialized as 0.
    testNaN_new = initNumVar( testNaN )

    #300.   Overwrite the input dataset with the numeric values initialized as 0.
    initNumVar( testNaN , inplace = True )
#-Notes- -End-
"""